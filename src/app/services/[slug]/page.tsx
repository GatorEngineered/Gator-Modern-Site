...
export default async function ServicePage({ params }: Props) {
  ...
  return (
    <main id="main-content" style={{ background: "var(--dark,#060d14)", minHeight: "100vh", paddingBottom: "6rem" }}>

      {/* Breadcrumb bar */}
      <div className="border-b" style={{ borderColor: "var(--border,#0d2137)", background: "var(--panel,#0a1520)" }}>
        ...
        >
          <Link href="/"
            className="transition-colors duration-150 hover:text-[#00d4ff]"
            style={{ color: "var(--muted,#4a6275)", textDecoration: "none" }}
          >Home</Link>
          <span>›</span>
          <Link href="/#services"
            className="transition-colors duration-150 hover:text-[#00d4ff]"
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        {/* Hero */}
        <div className="pt-16 pb-14" style={{ borderBottom: "1px solid var(--border,#0d2137)" }}>
          <div className="flex items-center gap-3 mb-5">
            <span className="text-xs px-3 py-1"
              style={{ fontFamily: "var(--font-mono,monospace)", color: "var(--teal,#00d4ff)", border: "1px solid var(--border,#0d2137)", background: "rgba(0,212,255,0.06)" }}
            >{svc.num}</span>
            {svc.tags.map(tag => (
              <span key={tag} className="text-xs px-2 py-1"
                style={{ fontFamily: "var(--font-mono,monospace)", color: "var(--teal-dim,#007fa3)", border: "1px solid var(--border,#0d2137)", background: "rgba(0,212,255,0.04)" }}
              >{tag}</span>
            ))}
          </div>

          <h1 className="font-bold mb-4"
            style={{ fontFamily: "var(--font-head,sans-serif)", fontSize: "clamp(2.2rem,5vw,3.8rem)", color: "var(--text,#c8dce8)", lineHeight: 1.08 }}
          >{svc.title}</h1>

          <p className="text-lg font-semibold"
            style={{ fontFamily: "var(--font-body,sans-serif)", color: "var(--teal,#00d4ff)", maxWidth: 640 }}
          >{svc.tagline}</p>
        </div>

        {/* Intro */}
        <section className="py-12" style={{ borderBottom: "1px solid var(--border,#0d2137)" }}>
          <p style={{ fontFamily: "var(--font-body,sans-serif)", color: "var(--text,#c8dce8)", fontWeight: 500, maxWidth: 760, fontSize: "clamp(1rem,1.8vw,1.15rem)", lineHeight: 1.75 }}
          >{svc.intro}</p>
        </section>
        ...
        <section className="py-12" style={{ borderBottom: "1px solid var(--border,#0d2137)" }}>
          ...
          >What It Is</p>
          <p style={{ fontFamily: "var(--font-body,sans-serif)", color: "var(--muted,#4a6275)", fontWeight: 500, maxWidth: 760, fontSize: "clamp(0.95rem,1.7vw,1.05rem)", lineHeight: 1.75 }}
          >{svc.what}</p>
        </section>
        ...
        <section className="py-12" style={{ borderBottom: "1px solid var(--border,#0d2137)" }}>
          ...
          <div className="p-6" style={{ background: "var(--panel,#0a1520)", border: "1px solid var(--border,#0d2137)", borderLeft: "3px solid var(--teal,#00d4ff)" }}>
          ...
          <div className="mt-5">
            <Link href="/#how-it-works"
              className="text-sm font-semibold transition-colors duration-150 hover:text-[#00ff88]"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {related.map(r => (
                <Link key={r.slug} href={`/services/${r.slug}`}
                  className="flex flex-col p-5 transition-colors duration-150 hover:border-[#007fa3]"
                  style={{
        ...
        <section className="py-16">
          ...
          >
            ...
            <h2 className="font-bold mb-3"
              style={{ fontFamily: "var(--font-head,sans-serif)", fontSize: "clamp(1.6rem,3.5vw,2.4rem)", color: "var(--text,#c8dce8)", lineHeight: 1.1 }}
            >Let&apos;s talk about {svc.title}.</h2>
            <p className="mb-8 mx-auto text-sm leading-relaxed"
              style={{ fontFamily: "var(--font-body,sans-serif)", color: "var(--muted,#4a6275)", fontWeight: 500, maxWidth: 440 }}
            >30–90 day engagement. Clear deliverables. Clean exit. Tell us what you need and we&apos;ll map out a scope.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/#contact"
                className="inline-flex items-center gap-2 px-8 py-4 text-sm font-bold transition-all duration-200 hover:bg-[#00ff88]"
                style={{
              ...
              <a href="mailto:reva@gatorengineered.com"
                className="text-sm font-semibold transition-colors duration-150 hover:text-[#00d4ff]"
